...
const ViewCleaningSection = ({ propertyId, selectedPropertyName }) => {
  const { cleaningData, isLoading, error, mutate } =
  ...
  const handleDelete = (id) => {
    const item = cleaningData.find((item) => item.id === id);
  ...
  return (
    <div className="p-6">
      <h3 className="text-xl font-bold text-dark mb-4">
        Cleaning History for {selectedPropertyName || "Selected Property"}
      </h3>

      {cleaningData.length === 0 ? (
        <p className="text-dark/70">No cleaning records found.</p>
      ) : (
          ...
          {cleaningData.map((item, index) => (
            ...
            >
              <div className="flex justify-between items-center">
                <span className="font-medium text-dark">{item.company}</span>
...
export default ViewCleaningSection;
