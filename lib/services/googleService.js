...
const googleService = {
  ...
  init: async (forceAuth = false) => {
    try {
      ...
      if (!tokens || !tokens.refresh_token) {
        console.log("Falling back to environment variables for tokens");
        tokens = {
      ...
      // CRITICAL FIX: Set credentials IMMEDIATELY after loading tokens
      ...
      if (!tokens.expiry_date || now > tokens.expiry_date - 60000) {
        ...
        try {
          ...
          // Update stored tokens if possible
          try {
            await UserService.updateSystemSettings({
              google_access_token: refreshed.credentials.access_token,
              google_token_expiry:
                refreshed.credentials.expiry_date?.toString(),
            });
          } catch (updateError) {
            console.warn("Could not update tokens in database:", updateError);
          }
        } catch (refreshError) {
  ...
  async findReceiptsFolder(folderId, monthYear) {
    try {
      ...
      // Format the month year to handle both formats
      const formattedMonthYear = monthYear.replace(/\s+/g, "");
      const spaceFormattedMonthYear = monthYear.match(/([a-zA-Z]+)(\d+)/)
        ? `${monthYear.match(/([a-zA-Z]+)(\d+)/)[1]} ${
            monthYear.match(/([a-zA-Z]+)(\d+)/)[2]
          }`
        : monthYear;

      const monthYearQuery = `'${parentFolderId}' in parents AND (name = '${formattedMonthYear.replace(
        /'/g,
        "\\'"
      )}' OR name = '${spaceFormattedMonthYear.replace(
        /'/g,
        "\\'"
      ...
      if (monthYearResponse.data.files.length > 0) {
        ...
        console.log(
          `Found existing subfolder '${monthYearResponse.data.files[0].name}' with ID: ${monthYearFolderId}`
        );
      } else {
        const monthYearFolderMetadata = {
          name: spaceFormattedMonthYear,
        ...
        console.log(
          `Created new subfolder '${spaceFormattedMonthYear}' with ID: ${monthYearFolderId}`
        );
      }
  ...
  findReceiptsSubfolder: async (parentFolderId, monthYear) => {
    // Format the month year to handle both formats
    const formattedMonthYear = monthYear.replace(/\s+/g, ""); // Remove spaces
    const spaceFormattedMonthYear = monthYear.match(/([a-zA-Z]+)(\d+)/)
      ? `${monthYear.match(/([a-zA-Z]+)(\d+)/)[1]} ${
          monthYear.match(/([a-zA-Z]+)(\d+)/)[2]
        }`
    ...
    const subfolderQuery = `'${parentFolderId}' in parents AND (name = '${formattedMonthYear.replace(
      /'/g,
      "\\'"
    )}' OR name = '${spaceFormattedMonthYear.replace(
      /'/g,
    ...
    if (subfolderResponse.data.files.length > 0) {
      ...
      console.log(
        `Found existing subfolder '${subfolderResponse.data.files[0].name}' with ID: ${monthYearFolderId}`
      );
    } else {
      // Create new folder with space format
      const monthYearFolderMetadata = {
        name: spaceFormattedMonthYear, // Use format with space
        mimeType: "application/vnd.google-apps.folder",
        parents: [parentFolderId],
      };
  ...
  // Example of how to use the limiter with your existing Google API calls
  async updateRangeValues(sheetId, sheetName, range, values) {
    try {
      // Use googleApiLimiter to schedule the API call
      return await googleLimiter.schedule(async () => {
        const formattedSheetName = sheetName.includes(" ")
          ? `'${sheetName}'`
          : sheetName;

        const formattedRange = `${formattedSheetName}!${range}`;
        console.log(`Updating range: ${formattedRange} with values:`, values);
