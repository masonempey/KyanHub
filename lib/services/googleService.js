...
const googleService = {
  ...
  startFolderWatch: async (folderId, webhookUrl) => {
    try {
      //using crypto library we generate a random UUID and assign to channelID
      const channelId = crypto.randomUUID();
      //expiration is set to 30 days from now
      //How can I get this to reset?
      const expiration = Date.now() + 30 * 24 * 60 * 60 * 1000;
      //calls the drive.files.watch method to start watching the folder with the given folderId
      const watchResponse = await drive.files.watch({
        fileId: folderId,
        requestBody: {
          //channelId is a random UUID: used for verification
          id: channelId,
          //web_hook enables real time monitor (watching effect)
          type: "web_hook",
          address: webhookUrl,
