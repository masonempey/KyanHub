...
const googleService = {
  ...
  updateRangeValues: async (sheetId, sheetName, range, values) => {
    try {
      // Format the sheet name to handle spaces and special characters
      const formattedSheetName =
        sheetName.includes(" ") ||
        sheetName.includes("-") ||
        sheetName.includes("'")
      ...
      console.log(
        `Updating ${formattedSheetName}!${range} with: ${JSON.stringify(
          values
        )}`
      );
      ...
      console.log(
        `Updated ${result.data.updatedCells} cells in range ${formattedSheetName}!${range}`
      );
  ...
  updateSheetValues: async (sheetId, sheetName, updates) => {
    try {
      ...
      const requests = updates.map(({ range, value }) => ({
        spreadsheetId: sheetId,
        range: `${formattedSheetName}!${range}`, // Use the formatted sheet name
        valueInputOption: "USER_ENTERED",
