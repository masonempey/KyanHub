...
class CleaningService {
  ...
  static async insertCompany(companyName, googleFolderId) {
    const client = await pool.connect();
    try {
      await client.query(
  ...
  static async getFolderIds() {
    const client = await pool.connect();
    try {
      const result = await pool.query(
        "SELECT google_folder_id FROM cleaning_companies"
      );
      return result.rows;
    } finally {
      client.release();
