  ...
  static async addProduct(name, ownerPrice, realPrice) {
    try {
      const result = await pool.query(
        `INSERT INTO products (name, owner_price, real_price, "order")
         VALUES ($1, $2, $3, (SELECT COALESCE(MAX("order"), -1) + 1 FROM products))
         RETURNING *`,
    ...
    try {
      ...
      if (inventoryResult.rows.length > 0) {
        ...
        const productsResult = await pool.query(
          'SELECT id, name, owner_price, real_price FROM products ORDER BY "order" ASC'
        );
