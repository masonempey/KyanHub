...
class MonthEndService {
  ...
  static async updateRevenueStatus(
    ...
    const client = await pool.connect();
    try {
      const revenueAfterCleaning = revenueAmount - cleaningAmount;
      const ownerRevenueShare = (revenueAfterCleaning * ownerPercentage) / 100;

      const ownerProfit = ownerRevenueShare - expensesAmount;
