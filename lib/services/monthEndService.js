...
class MonthEndService {
  ...
  static async updateRevenueAndStatus(params) {
    ...
    if (!["ready", "complete"].includes(status)) {
      throw new Error(
        `Invalid status for update: ${status}. Must be 'ready' or 'complete'`
      );
    }

    // Use client transaction to ensure updates are atomic
    const client = await pool.connect();
    try {
      ...
      if (existingRecord.rows.length > 0) {
        ...
        result = await client.query(
          ...
          status = $10,
          updated_at = CURRENT_TIMESTAMP
         WHERE property_id = $11 AND year = $12 AND month_number = $13
         RETURNING *`,
          [
            propertyName,
            revenueAmount,
